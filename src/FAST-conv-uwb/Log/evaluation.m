% result = load("20231204100011.txt");
...
folder = "/home/seu/c7map_for_wx";
% folder = "/mnt/sdb/Datasets/NCLT/datasets/no_LIO";
% folder = "/mnt/sdb/Datasets/NCLT/datasets/LOAM";

date = "2012-06-15";
% dateS = "2013-02-23withDownsamplePC";
% logFilePath = folder+"/"+date+"/20231204121054.txt"
% poseFilePath = folder+"/"+date+"/map_pcd/path_mapping.txt";
% poseFilePath = folder+"/"+date+"-0.5HZ/map_pcd/path_vinsfusion.txt";
poseFilePath = folder+"/"+date+"/20231204121054.txt"
poseFilePathrpy = folder+"/"+date+"/20231204121054rpy.txt"
gtFilePath = folder+"/"+date+"/groundtruth_"+date+".csv"

%%quat->euler
fid=fopen(poseFilePathrpy,'w+');
pose_quat = load(poseFilePath);
lenPose = length(pose_quat);
for i=1:lenPose
    [r1,r2,r3] = quat2angle([pose_quat(i,8),pose_quat(i,7),pose_quat(i,6),pose_quat(i,5)]);
     fprintf(fid,'%.6f %.4f %.4f %.4f %.6f %.6f %.6f %.6f\n',pose_quat(i,1),pose_quat(i,2),pose_quat(i,3),pose_quat(i,4),r1,r2,r3);
end
fclose(fid); 
