    ...
    for (uint i = 1; i < plsize; i++)
    {
      if ((msg->points[i].line < N_SCANS)) // && ((msg->points[i].tag & 0x30) == 0x10))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          if ((pl_full[i].intensity > 4) && (pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z > blind_sqr))
          {
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    if (i % point_filter_num == 0)
    {
      ...
      {
        pl_surf.points.push_back(added_pt);
         //printf("time mode: %d time: %f \n", given_offset_time, pl_orig.points[i].time);
      }
    }
  }
}
